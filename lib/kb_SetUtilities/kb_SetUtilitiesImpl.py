    ...
    def KButil_Merge_ReadsSet_to_OneLibrary(self, ctx, params):
        ...
        try:
            ...
            input_reads_obj_info = wsClient.get_object_info_new(
                {'objects':[{'ref':params['input_ref']}]})[0]
        ...
        try:
            setAPI_Client = SetAPI(url=self.serviceWizardURL, token=ctx['token'])
            input_readsSet_obj = setAPI_Client.get_reads_set_v1({'ref': params['input_ref'],
                                                                'include_item_info': 1})
        except Exception as e:
            raise ValueError('SetAPI Error: Unable to get read library set from workspace: (' +
                             str(params['input_ref']) + ")\n" + str(e))
        ...
        for input_reads_library_ref in readsSet_ref_list:
            ...
            try:
                ...
                [OBJID_I, NAME_I, TYPE_I, SAVE_DATE_I, VERSION_I, SAVED_BY_I, WSID_I, WORKSPACE_I,
                 CHSUM_I, SIZE_I, META_I] = range(11)

                input_reads_obj_info = wsClient.get_object_info_new(
                    {'objects': [{'ref': input_reads_library_ref}]})[0]
                input_reads_obj_type = input_reads_obj_info[TYPE_I]
            ...
            if read_library_type is None:
                read_library_type = input_reads_obj_type
            elif input_reads_obj_type != read_library_type:
                raise ValueError("incompatible read library types in ReadsSet " +
                                 params['input_ref'])
        ...
        for this_input_reads_ref in readsSet_ref_list:
            self.log(console, "DOWNLOADING FASTQ FILES FOR ReadsSet member: " +
            ...
            if sequencing_tech is None:
                sequencing_tech = this_sequencing_tech
            elif this_sequencing_tech != sequencing_tech:
                sequencing_tech = 'N/A'

            # append fwd
            self.log(console, "APPENDING FASTQ FILES FOR ReadsSet member: " +
                     str(this_input_reads_ref))
        ...
        if input_reads_obj_type == "KBaseFile.PairedEndLibrary":
            reads_library_ref = readsUtils_Client.upload_reads({
                'wsname': str(params['workspace_name']),
            ...
            reads_library_ref = readsUtils_Client.upload_reads({
                ...
                'source_reads_ref': readsSet_ref_list[0],
                'single_genome': 0,
                'fwd_file': combined_input_fwd_path})['obj_ref']

        # build report message
        report += "NUM READS LIBRARIES COMBINED INTO ONE READS LIBRARY: "
        report += str(len(readsSet_ref_list)) + "\n"
